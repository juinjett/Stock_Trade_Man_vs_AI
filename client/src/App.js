...
class App extends Component {
  ...
  getNewStock() {
    ...
    userStockData.userSoldList = [0, 0, 0];
    userStockData.userBoughtList = [0, 0, 0];
    mlStockData.mlSoldList = [0, 0, 0];
    mlStockData.mlBoughtList = [0, 0, 0];
    async.series([
      (callback) => {
    ...
    if (data.length !== currentData.length) {
      ...
      if (mlShouldBuySell) {
        if (mlShouldBuySell === 'buy') {
          currentMLScatterColor.push("rgba(255, 178, 102, 1)");
          ...
          mlStockData.mlBoughtList[2 - mlStockData.currentBuys] = lastStockPrice;
          console.log(mlStockData.mlBoughtList);
        }
        else if (mlShouldBuySell === 'sell') {
          currentMLScatterColor.push("rgba(178, 102, 255, 1)");
          currentMLScatterData.push(currentData[currentData.length-1]);
          mlStockData.currentSells--;
          mlStockData.currentStocks--;
          mlStockData.bank = (parseFloat(mlStockData.bank) + lastStockPrice).toFixed(2);
          mlStockData.currentStockValue -= lastStockPrice;
          mlStockData.mlSoldList[2 - mlStockData.currentSells] = lastStockPrice;
          console.log(mlStockData.mlSoldList);
        }
      }
      if (this.state.userSold) {
        ...
        userStockData.userSoldList[2 - userStockData.currentSells] = lastStockPrice;
        ...
        userStockData.bank = (parseFloat(userStockData.bank) - lastStockPrice).toFixed(2);
        userStockData.currentStockValue += lastStockPrice;
        userStockData.userBoughtList[2 - userStockData.currentBuys] = lastStockPrice;
  ...
  render() {
    ...
    var userStockData = this.state.userStockData;
    var mlStockData = this.state.mlStockData;
    var lastStockPrice = this.state.lastStockPrice;
    ...
    var introJSX = [];
    var startJSX = [];
    if (loggedIn == false && signedUp == false) {
      loginJSX.push(
        <div id="login">
          <h2>Stock Trading: Man vs AI</h2>
          <h4>Please log in</h4>
          <form onSubmit={this.handleSubmit}>
            <label for="email">Email: </label>
            <br />
            <input id="email" type="email" name="email" value={this.state.value} onChange={this.handleChange} placeholder="your email" required></input>
            <br />
            <label for="Password">Password: </label>
            <br/>
            <input id="Password" type="password" name="Password" value={this.state.value} onChange={this.handleChange} placeholder="your password" pattern=".{5,10}" required title="5 to 10 characters"></input>
            <br />
            <br/>
            <button onClick={() => {this.handleLoggin()}} id="start-btn" style={{"margin-right": "20px"}} className="btn btn-active" >Login</button>
            <button onClick={() => {this.handleSignup()}} id="start-btn" style={{"margin-left": "20px"}} className="btn btn-active" >Sign Up</button>
            <br />
          </form>
        </div>
      );
    ...
    else if (loggedIn == false && signedUp){
      signupJSX.push(
        <div id="login">
          <h2>Stock Trading: Man vs AI</h2>
          ...
          <form onSubmit={this.handleSubmit}>
            ...
            <br/>
            <br/>
            <button onClick={() => {this.handleLoggin()}} id="start-btn" style={{"margin-right": "20px"}} className="btn btn-active" >Submit</button>
            <br />
          </form>
        </div>
      );
    ...
    if (svgJSX.length > 0) {
      ...
      introJSX.push(
        <div id="intro">
          <h2>Stock Trading: Man vs AI</h2>
          <h4>This is a little game to test your stock trading skills against the market and a machine learning algorithm</h4>
          <p> A random stock will be given with a random 365-day period of time. You and the AI will each do 3 "buys" and 3 "sells" in the period. Press the up arrow key to "buy" a stock, and press the down arrow key to "sell" a stock.</p>
          <br />
          <p>Try your best to beat AI!</p>
          <br />
        </div>
      );
      startJSX.push(
