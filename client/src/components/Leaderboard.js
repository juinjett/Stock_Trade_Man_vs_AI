  ...
  render() {
    ...
    if (records.gamesPlayed >= 1 || (data.length > 0 && currentData.length >= data.length-1)) {
      ...
      if (!this.props.leaderboardIsHidden) {
        ...
        leaderboardContent = (
          <div>
            <h3 id="lb-heading" style={{"color":"white"}} >ScoreRank</h3>
            <div className="leader-content-container">
              <div className="leader-content">
                <p><span style={{"font-weight":"bold", "color":"white"}}>1st {records.leaderboard[0].name}: {records.leaderboard[0].score}</span></p>
                <p><span style={{"font-weight":"bold", "color":"white"}}>2nd {records.leaderboard[1].name}: {records.leaderboard[1].score}</span></p>
                <p><span style={{"font-weight":"bold", "color":"white"}}>3rd {records.leaderboard[2].name}: {records.leaderboard[2].score}</span></p>
              </div>
            </div>
          </div>
        );
